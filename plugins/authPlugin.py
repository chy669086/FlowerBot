                ...
                if args[1] == "blacklist":
                    if is_admin(self.event.sender.id):
                        ...
                        if args[2] == "add":
                            ...
                            if is_admin(target):
                                await self.event.reply(
                                    "可是这是一位管理，我不能无视他的指令".format()
                                )
                                return
                            to_blacklist(target)
                            await self.event.reply(
                                "哦哦，好的。不再响应 {:s} 的指令。".format(target)
                            )
                        elif args[2] == "remove":
                            remove_blacklist(target)
                            await self.event.reply(
                                "哦哦，好的。开始响应 {:s} 的指令。".format(target)
                            )
                    else:
                        await self.event.reply("没有权限")
                ...
                if args[1] == "admin":
                    if is_superadmin(self.event.sender.id):
                        ...
                        if args[2] == "add":
                            to_admin(target)
                            await self.event.reply(
                                "哦哦，好的。已经将 {:s} 设置为管理".format(target)
                            )
                        elif args[2] == "remove":
                            remove_admin(target)
                            await self.event.reply(
                                "哦哦，好的。已经将 {:s} 取消管理".format(target)
                            )
                    else:
                        await self.event.reply("没有权限")
