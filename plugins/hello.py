...
HelloMessage = """你好!我是呵呵哒，一个 Codeforces/Atcoder Duel Bot，你可以在这里绑定你的 CF 
...
VersionMessage = """最近更新于 2024/12/31
daily_problem 使用数据库存储
"""
...
class Hello(Plugin):
    ...
    async def handle(self) -> None:
        ...
        if args[0] == "/help" or args[0] == "/helpduel":
            if len(args) == 1:
                ...
                if not requestauth(self.event.sender.group.id, plug):
                    ...
                    if plug in manuel:
                        msg = gen_quote(
                            "{:s} 模块的帮助".format(plug), "帮助信息", manuel[plug]
                        )
                    else:
        ...
        if args[0] == "/sayto":
            if is_admin(self.event.sender.id):
                ...
                await self.event.adapter.sendGroupMessage(
                    target=int(target), messageChain=p
                )
    ...
    async def rule(self) -> bool:
        try:
            ...
            return (
                text
                in ["/hello", "/heartbeat", "/命运的天秤", "/test", "/news", "/temp"]
                or text.startswith("/exec")
